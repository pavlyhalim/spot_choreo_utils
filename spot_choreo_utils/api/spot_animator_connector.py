...
def set_robot_references(spot_instance, plant_context_instance, context_instance, meshcat_instance):
...
def update_robot_config_from_joint_angles(joint_angles):
    """Updates the robot configuration based on joint angles"""
    global spot, plant_context, context
    if not spot or not plant_context or not context:
        logger.error("Robot references not set, cannot update pose")
        return False
    
    try:
        from web_animator.spot_web_animator.animation_backend.web_animator_backend import update_robot_config_from_joint_angles as update_func
...
async def create_animation(request: NewAnimationRequest):
...
async def load_animation(name: str):
...
async def save_keyframe(name: str, request: SaveKeyframeRequest):
...
async def update_keyframe_api(name: str, index: int, request: UpdateKeyframeRequest):
    ...
    try:
        if active_animation and active_animation.name == name:
            ...
            if keyframe:
                ...
                return {
                    "index": index,
                    "time": active_animation.animation_keyframes[index].time
                }
            else:
...
async def name_segment(name: str, request: NameSegmentRequest):
...
async def adjust_timing(name: str, request: AdjustTimingRequest):
    ...
    try:
        if active_animation and active_animation.name == name:
            ...
            # Adjust timing
            success = adjust_animation_timing(
                active_animation, 
                request.keyframe_indices,
                request.new_timestamps
            )
...
async def stop_animation(name: str):
        ...
        while True:
            ...
            if "get_meshcat_url" in data:
                ...
                await websocket.send_json({
                    "type": "meshcat_url",
                    "url": meshcat_url  # Use global variable
                })
                continue
